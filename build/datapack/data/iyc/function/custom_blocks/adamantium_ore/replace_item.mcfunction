...
#> iyc:custom_blocks/adamantium_ore/replace_item
#
# @within	iyc:custom_blocks/adamantium_ore/destroy
#

# If silk touch applied
execute if score #is_silk_touch iyc.data matches 1 run data modify entity @s Item.id set from storage iyc:items all.adamantium_ore.id
execute if score #is_silk_touch iyc.data matches 1 run data modify entity @s Item.components set from storage iyc:items all.adamantium_ore.components
