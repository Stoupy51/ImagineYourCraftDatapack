
#> iyc:utils/get_all_recipes
#
# @within	???
#

# Get all recipes
recipe give @s iyc:emerald_helmet
recipe give @s iyc:emerald_chestplate
recipe give @s iyc:emerald_leggings
recipe give @s iyc:emerald_boots
recipe give @s iyc:emerald_sword
recipe give @s iyc:emerald_pickaxe
recipe give @s iyc:emerald_axe
recipe give @s iyc:emerald_shovel
recipe give @s iyc:emerald_hoe
recipe give @s iyc:stone_stick
recipe give @s iyc:massive_obsidian_block
recipe give @s iyc:iron_furnace
recipe give @s iyc:blue_block_ctf
recipe give @s iyc:bolt
recipe give @s iyc:clear_glass
recipe give @s iyc:helice_hat
recipe give @s iyc:iron_ladder
recipe give @s iyc:birch_wood_lantern
recipe give @s iyc:jungle_wood_lantern
recipe give @s iyc:oak_wood_lantern
recipe give @s iyc:spruce_wood_lantern
recipe give @s iyc:iron_lantern
recipe give @s iyc:red_light
recipe give @s iyc:reversed_oak_planks
recipe give @s iyc:beer_mug
recipe give @s iyc:beer_mug_2
recipe give @s iyc:empty_glass
recipe give @s iyc:toast
recipe give @s iyc:glass_pot
recipe give @s iyc:hot_chocolate
recipe give @s iyc:chainmail

