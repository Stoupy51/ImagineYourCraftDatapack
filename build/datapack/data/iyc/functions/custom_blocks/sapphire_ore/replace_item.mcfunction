...
#> iyc:custom_blocks/sapphire_ore/replace_item
#
# @within	iyc:custom_blocks/sapphire_ore/destroy
#

# If silk touch applied
execute if score #is_silk_touch iyc.data matches 1 run data modify entity @s Item.id set from storage iyc:items all.sapphire_ore.id
execute if score #is_silk_touch iyc.data matches 1 run data modify entity @s Item.components set from storage iyc:items all.sapphire_ore.components
