...
execute if score #dependency_error iyc.data matches 1 run tellraw @a {"translate":"iyc.imagineyourcraft_error_libraries_are_missingplease_download_thee:","color":"red"}
...
execute if score #dependency_error iyc.data matches 1 unless score #smithed.crafter.major load.status matches 0.. unless score #smithed.crafter.minor load.status matches 3.. unless score #smithed.crafter.patch load.status matches 1.. run tellraw @a {"translate":"iyc.smithed_crafter","color":"gold","clickEvent":{"action":"open_url","value":"https://wiki.smithed.dev/libraries/crafter/"}}
