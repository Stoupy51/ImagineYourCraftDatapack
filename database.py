...
for ore in ores:
	...
	if f"{ore}_block.png" in textures_filenames:
		with open(f"{TEXTURES_FOLDER}/{ore}_block.png", "rb") as file:
			...
			print(f"Color for {ore} block: {color}")
	...
	for placeable in placeables:
		...
		if placeable == "block":
			if f"{material}_ingot.png" in textures_filenames:
				DATABASE[block][CRAFTING_RECIPES].append(str({"type":"shapeless", "result_count": 1, "ingredients": [ingredient_repr(f"{ore}_ingot", count = 9)]}))
			if f"{material}_fragment.png" in textures_filenames:
				DATABASE[block][CRAFTING_RECIPES].append(str({"type":"shapeless", "result_count": 1, "ingredients": [ingredient_repr(f"{ore}_fragment", count = 9)]}))
			if f"{material}.png" in textures_filenames:
				DATABASE[block][CRAFTING_RECIPES].append(str({"type":"shapeless", "result_count": 1, "ingredients": [ingredient_repr(ore, count = 9)]}))
		pass
	...
	for ingredient in ingredients:
		...
		if ingredient == "ingot" or ingredient == "":
			if f"{ore}_block.png" in textures_filenames:
				DATABASE[item][CRAFTING_RECIPES].append(str({"type":"shapeless", "result_count": 9, "ingredients": [ingredient_repr(f"{ore}_block", count = 1)]}))
			if f"{ore}_nugget.png" in textures_filenames:
				DATABASE[item][CRAFTING_RECIPES].append(str({"type":"shapeless", "result_count": 1, "ingredients": [ingredient_repr(f"{ore}_nugget", count = 9)]}))
			if f"raw_{ore}.png" in textures_filenames:
				DATABASE[item][CRAFTING_RECIPES].append(str({"type":"smelting", "result_count": 1, "ingredient": ingredient_repr(f"raw_{ore}")}))
			if f"{ore}_dust.png" in textures_filenames:
				DATABASE[item][CRAFTING_RECIPES].append(str({"type":"smelting", "result_count": 1, "ingredient": ingredient_repr(f"{ore}_dust")}))
			if f"{ore}_ore.png" in textures_filenames:
				DATABASE[item][CRAFTING_RECIPES].append(str({"type":"smelting", "result_count": 1, "ingredient": ingredient_repr(f"{ore}_ore")}))
		pass
	...
	for gear in equipments:
		...
		if gear == "helmet":
			craft_gear = {"type": "shaped", "result_count": 1, "shape": "XXXX X", "ingredients": {"X": ingredient_repr(ingr)}}
		elif gear == "chestplate":
			craft_gear = {"type": "shaped", "result_count": 1, "shape": "X XXXXXXX", "ingredients": {"X": ingredient_repr(ingr)}}
		elif gear == "leggings":
			craft_gear = {"type": "shaped", "result_count": 1, "shape": "XXXX XX X", "ingredients": {"X": ingredient_repr(ingr)}}
		elif gear == "boots":
			craft_gear = {"type": "shaped", "result_count": 1, "shape": "X XX X", "ingredients": {"X": ingredient_repr(ingr)}}
		elif gear == "sword":
			craft_gear = {"type": "shaped", "result_count": 1, "shape": "X  X  S", "ingredients": {"X": ingredient_repr(ingr), "S": {"id":"minecraft:stick"}}}
		elif gear == "pickaxe":
			craft_gear = {"type": "shaped", "result_count": 1, "shape": "XXX S  S ", "ingredients": {"X": ingredient_repr(ingr), "S": {"id":"minecraft:stick"}}}
		elif gear == "axe":
			craft_gear = {"type": "shaped", "result_count": 1, "shape": "XX XS  S ", "ingredients": {"X": ingredient_repr(ingr), "S": {"id":"minecraft:stick"}}}
		elif gear == "shovel":
			craft_gear = {"type": "shaped", "result_count": 1, "shape": "X  S  S  ", "ingredients": {"X": ingredient_repr(ingr), "S": {"id":"minecraft:stick"}}}
		elif gear == "hoe":
